...
theme.Cart = (function() {
  ...
  Cart.prototype = Object.assign({}, Cart.prototype, {
    ...
    _createCart: function(state) {
      ...
      var cartprice = state.total_price;
      getCart(cartprice);
    },
...
// Add multiple product JS

$(document).ready(function(){
  var itemsList =[];
  $('#add').click(function(){
    var selcted_variant_id = $('#ProductSelect-custom-add-to-cart option:selected').val();
    var id = $('.custom-product-image img').attr('product-id');
    var item = {};
    var itemtwo = {};
    item.id = selcted_variant_id;
    item.quantity =1;    
    itemtwo.id = id;
    itemtwo.quantity = 1;
    itemsList.push(item,itemtwo);
    var data = {
      "items": itemsList
    }; 
    console.log(data);
    if($('#prdtimg').is(":checked")){
      $.ajax({
        type: 'POST', 
        url: '/cart/add.js',
        data: data,
